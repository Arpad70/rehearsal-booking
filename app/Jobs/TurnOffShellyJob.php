...
class TurnOffShellyJob implements ShouldQueue {  
    ...
    public function handle(): void {  
        ...
        if (!$room->shelly_ip) {
            Log::warning('TurnOffShellyJob: Room has no Shelly IP configured', [
                'reservation_id' => $reservation->id,
                'room_id' => $room->id,
            ]);
            return;  
        ...
        if (!$gateway) {
            Log::warning('TurnOffShellyJob: No Shelly gateway URL configured', [
                'reservation_id' => $reservation->id,
            ]);
            return;  
        }
        ...
        try {  
            ...
            if ($response->successful()) {
                Log::info('TurnOffShellyJob: Successfully sent turn_off request', [
                    'reservation_id' => $reservation->id,
                    'room_id' => $room->id,
                ]);
            } else {
                Log::warning('TurnOffShellyJob: Gateway returned error status', [
                    'reservation_id' => $reservation->id,
                    'room_id' => $room->id,
